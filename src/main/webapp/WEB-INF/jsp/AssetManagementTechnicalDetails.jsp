...
$(function() {
	var checkListApp = angular.module('detailsApp', []);
	checkListApp.controller('detailsCntr',function($scope, $http ) {
		
		$scope.detailsList=[];		
		$http.post("getAssetTechnicalDetail").success(function(data){	
			debugger;
		...
		$scope.updateDetails=function(){				
			$http.post("updateAssetTechnicalDetail",$scope.detailsList).success(function(data){			
				$scope.detailsList=data;
				$scope.confermeSave();
			}).error(function(){
				$scope.confermeFail();
...
<body ng-app="detailsApp" ng-controller="detailsCntr">
	<div class="intro-header">
		<h2 style="text-align: center">Asset Management Technical Details</h2>
		<br> <br>
				...
				<table class="table table-bordered table-striped">
					<thead>
						<tr>
							...
							<th class="tbh">Height</th>
							<th class="tbh">Width</th>
							<th class="tbh">Depth</th>
							<th class="tbh">Weight</th>	
							<th class="tbh">Color</th>											
						</tr>
					</thead>
					<tbody>
						<tr ng-repeat="details in detailsList | orderBy:'assetId' |filter:search">
							<td>{{$index+1}}</td>
							<td>{{details.assetId}}</td>
							<td>{{details.assetCode}}</td>
							<td><input type="text" class="form-control" ng-model="details.height" value="{{details.height}}"></td>
							<td><input type="text" class="form-control" ng-model="details.width" value="{{details.width}}"></td>
							<td><input type="text" class="form-control" ng-model="details.depth" value="{{details.depth}}"></td>
							<td><input type="text" class="form-control" ng-model="details.weight" value="{{details.weight}}"></td>
							<td><input type="text" class="form-control" ng-model="details.color" value="{{details.color}}"></td>
						</tr>
					</tbody>
					<tfoot>
						<tr>
							<td colspan="8" align="right">
								<button type="submit" ng-click="updateDetails()" class="btn btn-success">Update</button>
